    ...
    <div>
      <MobileRecipiesMenuContext>
        {isOpen && (
          <Disclosure.Panel
            static
            className="fixed top-0 z-20 h-screen w-full bg-gray-900 bg-opacity-60 lg:hidden"
          >
            {({ close }) => (
              <MobileMenuContext>
...
function MobileMenuLogicalComponent({ close }: { close: () => void }) {
  ...
  return (
    <>
      <div className=" relative h-full w-10/12 bg-white md:w-6/12">
        {/* Header */}
        ...
        {/* Search */}
        <div className=" mx-4 mt-4 flex items-center gap-2 border-b">
          <BiSearch size="1.2rem" className=" text-gray-500" />
          <input
            type="text"
            className="w-full border-0 px-0 focus:ring-0"
            placeholder="Search..."
          />
        </div>
        {openState === "notification" ? (
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gray-100 p-4">
          <div className=" flex items-center gap-4">
            <div className=" relative">
              <CircleImage src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" />
              <span className=" absolute right-0 bottom-0 block h-2 w-2 rounded-full bg-green-600"></span>
            </div>
            <div className=" flex flex-col">
              <h3 className=" text-sm font-bold text-gray-600">Ryan Ebner</h3>
